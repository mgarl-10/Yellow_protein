...
countdata<-read.table("Stammera_raw_counts.txt", header=T, row.names=1)
head(countdata)
ncol(countdata)
...
CTL<-results(dds, contrast=c("Treatment","YELL_UNT","YELL_HUM"))
...
dds_yell <- DESeq(dds_yell, fitType="mean")

keep <- rowSums(counts(dds_yell)) > 5
dds_yell <- dds_yell[keep,]
dds_yell <- DESeq(dds_yell, fitType="mean")
dds_yell$Treatment <- relevel(dds_yell$Treatment, ref = "YELL_HUM")
res_yell <- results(dds_yell)
summary (res_yell)
res_yell <- res_yell[order(res_yell$padj),]
vsd_yell <- varianceStabilizingTransformation(dds_yell, blind=TRUE, fitType="mean")
plotPCA(vsd_yell, intgroup="Treatment")

write.csv( as.data.frame(res_yell), file="yell_DEGs.csv")
...
coldata_yell <- coldata_yell[rownames(coldata_yell) != "YELL_HUM_R3", ]
countdata_yell <- countdata[, rownames(coldata_yell)]
...
dds_GFP <- DESeq(dds_GFP, fitType="mean")
res_GFP <- results(dds_GFP)
summary (res_GFP)
res_GFP <- res_GFP[order(res_GFP$padj),]
vsd_GFP <- varianceStabilizingTransformation(dds_GFP, blind=TRUE, fitType="mean")
plotPCA(vsd_GFP, intgroup="Treatment")
plotPCA(vsd_GFP, intgroup=c("Treatment", "Replicate"))
...
dds_CTL <- DESeq(dds_CTL, fitType="mean")
res_CTL <- results(dds_CTL)
summary (res_CTL)
res_CTL <- res_CTL[order(res_CTL$padj),]
vsd_CTL <- varianceStabilizingTransformation(dds_CTL, blind=TRUE, fitType="mean")
plotPCA(vsd_CTL, intgroup="Treatment")
plotPCA(vsd_CTL, intgroup=c("Treatment", "Replicate"))
